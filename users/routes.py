...
def following_list(user_id):
    ...
    query = db.session.query(User.id, User.username).filter(User.id.in_([u.id for u in user.followed]))
    results = query.all()
...
def follower_list(user_id):
    ...
    user = User.query.filter_by(id=user_id).first()
    query = db.session.query(User.id, User.username).filter(User.id.in_([u.id for u in user.followers]))
    results = query.all()
...
def search():
    ...
    users = db.session.query(User.id, User.username).filter(User.username == name)
